...
SECTIONS = [
    ...
    {
        "title": "Life Context",
        "summary": "The daily context that shapes health, access, and follow-through.",
        "questions": [
    ...
    {
        "title": "Care Team",
        "summary": "Current providers and coordination preferences.",
        "questions": [
    ...
    },
    {
        ...
        "summary": "Past and current diagnoses, hospital use, and unresolved clinical patterns.",
        "questions": [
            ...
            ("specialist_followups_pending", "Do you have any pending specialist follow-ups, scans, or tests?", "area", {}),
...
def collect_payload():
    ...
    data["meta_schema_version"] = "prvnt-comprehensive-intake-v2"
    return data
...
def render_question(key, label, qtype, cfg):
    ...
    if qtype == "text":
        ...
        result = st.number_input(
            ...
            value=value if value is not None else cfg.get("default", 0),
        )
    elif qtype == "date":
        ...
        result = st.slider(
            ...
            max_value=cfg.get("max", 10.0),
            value=value if value is not None else cfg.get("default", 0.0),
...
def store_submission(data):
    database_url = os.getenv("PRVNT_DATABASE_URL") or st.secrets.get("PRVNT_DATABASE_URL", None)
    encryption_key = os.getenv("PRVNT_FERNET_KEY") or st.secrets.get("PRVNT_FERNET_KEY", None)
